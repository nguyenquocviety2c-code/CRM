...
import type { WarehouseTransferResponse } from "@/types/report-warehouse";
import { cn } from "@/lib/utils";

interface Props {
  status: WarehouseTransferStatus;
  onStatusChange: (s: WarehouseTransferStatus) => void;
}

const subTabs: { key: WarehouseTransferStatus; label: string }[] = [
  { key: "completed", label: WarehouseTransferStatusLabel.completed },
  { key: "pending", label: WarehouseTransferStatusLabel.pending },
];
...
export function WarehouseTransferView({ status, onStatusChange }: Props) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      params.set("view", "transfer");
      params.set("transferStatus", status);
      ...
      return json.data as WarehouseTransferResponse;
    },
  });
  ...
  return (
    <div className="space-y-4">
      {/* Sub-tabs: Hoàn thành / Đang chuyển */}
        ...
        <Table className="w-full">
          <TableHeader>
            <TableRow className="bg-slate-50">
              ...
              <TableHead className="text-xs font-semibold uppercase text-slate-600 whitespace-nowrap">Kho Xuất</TableHead>
              <TableHead className="text-xs font-semibold uppercase text-slate-600 whitespace-nowrap">Kho nhận</TableHead>
              ...
              items.map((row) => (
                <TableRow key={row.id} className="hover:bg-slate-50">
                  ...
                  <TableCell>
                    ...
                    >
                      {row.status === "completed" ? "Hoàn thành" : "Đang chuyển"}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-slate-700">{row.fromBranchName}</TableCell>
                  <TableCell className="text-slate-700">{row.toBranchName}</TableCell>
