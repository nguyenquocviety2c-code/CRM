...
import {
  ...
  useAddReceipt,
  useRevenueVoucherCategories,
} from "@/stores/revenue-voucher-store";
import { validateCreateReceipt } from "@/lib/revenue-voucher-utils";
import { CreateReceiptInput } from "@/types/revenue-voucher";
...
export function CreateReceiptDialog() {
  ...
  const { data: categories } = useRevenueVoucherCategories();
  ...
  const handleChange = (field: keyof CreateReceiptInput, value: string | number | null) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = validateCreateReceipt(formData);
    if (!result.valid) {
    ...
    try {
      ...
      toast({
        title: "Lỗi",
        description:
          err instanceof Error ? err.message : "Tạo phiếu thu thất bại",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  return (
    <Dialog open={isCreateReceiptOpen} onOpenChange={handleClose}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="code"
                placeholder="Để trống để tự sinh (PTxxxxxx)"
                value={formData.code || ""}
                disabled={submitting}
