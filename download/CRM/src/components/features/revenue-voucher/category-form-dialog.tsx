...
import { validateCategoryName } from "@/lib/revenue-voucher-utils";
...
export function CategoryFormDialog() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Đã thêm",
        description: `Loại phiếu thu "${name.trim()}" đã được tạo`,
      });
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Tạo loại phiếu thu thất bại"
      );
    } finally {
      setSubmitting(false);
    }
