...
/**
 * POST /api/supabase/cashcards/[id]/extend
 * Body: { newExpiryDate, note? }
 *
 * Mirrors the Prisma route: sets expiry_date to the new value.
 */
export async function POST(req: NextRequest, { params }: Params) {
  try {
    const { id } = await params;
    const body = await req.json();
    const result = cashcardExtendSchema.safeParse(body);
    if (!result.success) {
      return NextResponse.json({ ok: false, error: result.error.issues[0].message }, { status: 400 });
    }

    const { newExpiryDate } = result.data;
    ...
    const { data, error: fetchErr } = await fetchCardWithRelations(id);
    if (fetchErr) {
